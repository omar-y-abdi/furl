...
per case, each case is measured in its own freshly-spawned Python process via
this worker. A fresh process means a fresh pipeline singleton and therefore a
fresh, empty Rust CCR store. No warm cache is carried between runs.

Invoked as::

    python -m verify.heldout.worker '<json-spec>'

where the spec is ``{"family","size","tier","seed","needles"}``. Emits a single
JSON line on stdout: the ``CaseResult`` as a dict.
"""
