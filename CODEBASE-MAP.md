...
> kept its responsibilities and now sits at ~2344 LOC — the extraction relieved
...
**ContentRouter extracted seams (6 clean seams lifted; orchestrator core stayed coupled — now ~2344 LOC)**
...
- **Prompt-cache ordering / byte-fidelity:** `headroom/compress.py:163` (`_compute_frozen_message_count` — only `messages[].content` `cache_control` blocks bump the frozen floor; system/tools always hot). Python prefix-stability is held by `CacheAligner.apply` (`cache_aligner.py:265`), which never reorders/rewrites the frozen prefix and tracks `_previous_prefix_hash` (`cache_aligner.py:229`). Enforced by `tests/test_cache_aligner_prefix_hash.py`, `tests/test_cache_aligner_hardening.py`, `tests/test_compress_frozen_prefix.py`.
...
- **apply() kwargs allowlist (typo guard):** `ContentRouter.apply` (`content_router.py:1430`) rejects any kwarg not in the module-level `_APPLY_ALLOWED_KWARGS` frozenset (`content_router.py:501`), so a misspelled per-request option fails loud instead of being silently ignored. Per-request options (`target_ratio`/`force_kompress`/`kompress_model`/`compression_policy`) are read off kwargs into the frozen `RouterRuntime` value via `RouterRuntime.from_kwargs` (`content_router.py:118`) and threaded by argument down compress/dispatcher/`_try_ml_compressor`/`_record_to_toin` — no thread-local, no main→worker replay (workers receive the same immutable value by argument).
