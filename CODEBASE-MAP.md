...
- `persist.rs:214` — `persist_dropped` — per-row chunks + row-index FIRST, whole-blob LAST, emit `<<ccr:HASH N_rows_offloaded>>` + `<<ccr:HASH#rows N_chunks>>`.
- `persist.rs:34` — `ccr_sentinel_map` — build `{_ccr_dropped, _ccr_rows?}` sentinel (recovery pointer unconditional on drop).
...
- `ccr/markers.rs:37/44/54/61/98` — `marker_for_rows_offloaded`/`marker_for_row_index`/`marker_for_opaque`/`marker_for_diff`/`marker_for_retrieve_more` — the SINGLE construction point for every Rust marker. Owns the *grammar*, not the hash: producers compute their own key and pass `hash` in. Every Rust producer routes through here, pinned byte-for-byte by the in-module equivalence tests (`markers.rs:88-163`).
...
- `crates/furl-py/src/lib.rs:872/930/1036` — `PySmartCrusher` / `crush` / `crush_array_json` — PyO3 bridge (GIL-released, validates at boundary).
...
- Touch CCR offload / sentinel → `persist.rs:214` (persist_dropped, write order), `persist.rs:34` (ccr_sentinel_map shape), row-index chunk marker via `marker_for_row_index` (`markers.rs:44`).
...
- Change content routing / per-type dispatch → `content_router.py:701` (ContentRouter.compress orchestrator), `content_router.py:814` (`_apply_strategy_to_content`), `content_router.py:223` (detect), `transforms/detection.rs` (Rust `detect` chain + `ContentType`; the regex `content_detector.rs` parity mirror was deleted).
...
- **Recovery invariant (no data loss):** marker emission is UNCONDITIONAL on drop — `persist.rs:214` (persist_dropped writes store + emits marker regardless of `enable_ccr_marker`). Verified Rust: `tests/ccr_roundtrip.rs:177` (distinct_inputs_produce_distinct_store_entries), `:364` (nested_array_inside_object_gets_marker_injected); lossless-win-no-write at `ccr_roundtrip.rs:117`. Verified Python: `tests/test_ccr_recovery_invariant.py:216` (marker-off surfaces pointer), `:260` (opaque-blob recovers), `:290` (lossy survivor table), `:119` (`_recover_from_output` across Rust `ccr_get` + Python `py_store.retrieve`). Round-trip decoder: `csv_schema_decoder.py` (`split_unquoted:259`, `_parse_iso:158`) / `verify/independent_recheck.py` (strict, no substring fallback). COR-14 caveat: the encoder's nested-uniform flatten is unrecorded on the wire, so decoded rows carry dotted top-level keys — reconstruction is value-exact under dotted keys, and the recheck compares both sides un-flattened (`independent_recheck._unflatten_dotted`).
...
- **apply() kwargs allowlist (typo guard):** `ContentRouter.apply` (`content_router.py:954`) rejects any kwarg not in the module-level `_APPLY_ALLOWED_KWARGS` frozenset (`content_router.py:481`), so a misspelled per-request option fails loud instead of being silently ignored.
