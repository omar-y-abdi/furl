...
def test_known_limitations_documented() -> None:
    """F4/F5/F8: the plugin README carries a Known-limitations section naming the
    heredoc divergence, the redaction gaps (incl. the tempfile transit), the
    Bash(...) allowlist mismatch, the cold-start uv resolves, and the cosmetic
    `line N:` prefix."""
    text = _read(_README)
    assert "Known limitations" in text
    section = text.split("Known limitations", 1)[1].lower()
    for needle in ("heredoc", "redaction", "tempfile", "allowlist", "uv", "line n"):
