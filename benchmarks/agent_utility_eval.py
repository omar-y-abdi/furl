...
def check_gt_present(archetype: str, gt: dict, blob: str) -> bool:
    ...
    if archetype == "anomaly":
        ...
        if gt.get("sample"):
            snippet = (gt["sample"][0].get("content", "") if isinstance(gt["sample"][0], dict)
                       else str(gt["sample"][0]))[:60]
...
def print_summary(all_records: list[dict]) -> None:
    ...
    hdr = (f"{'Corpus':<22} {'Bytes':>11} {'Ratio':>7} {'BlobCh':>8} "
    ...
    for cid, info in corpora.items():
        ...
        print(f"{cid:<22} {info['corpus_bytes']:>11,} {ratio_pct:>7} "
