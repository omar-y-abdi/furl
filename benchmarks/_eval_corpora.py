...
def build_app_log(seed: int = SEED) -> tuple[str, dict, str]:
    ...
    services = ["auth-svc", "api-gateway", "db-pool", "cache-svc",
                "worker", "scheduler", "notifier", "metrics-svc"]
    info_msgs = [
    ...
    for i, pos in enumerate(anomaly_positions):
        ...
        fmt_args = dict(
            host=rng.choice(hosts), n=rng.randint(3, 10),
            svc=rng.choice(services), line=rng.randint(100, 999),
            uid=f"u{rng.randint(1000,9999)}", ms=rng.randint(5000, 30000),
            table=rng.choice(tables), pct=rng.randint(85, 99),
            ep=rng.choice(["search", "export", "bulk"]),
            file=f"wal_{rng.randint(1,99):04d}.log",
            errno=rng.choice(["EIO", "ENOSPC", "EACCES"]),
            days=rng.randint(1, 14), req=rng.randint(1000, 5000),
            key=f"k:{rng.randint(1,999)}", sz=rng.randint(100, 9999),
            span_id=f"{rng.randint(1000,9999):x}", max=50, id=rng.randint(1, 99),
    ...
    base_ts = 0
    lines = []
    base_dt = datetime(2026, 6, 1, 0, 0, 0, tzinfo=timezone.utc)
    for i in range(5000):
        ...
        if i in anomaly_entries:
            ...
            msg = tmpl.format(
                key=f"k:{rng.randint(1,999)}", ms=rng.randint(1, 200),
                uid=f"u{rng.randint(1000,9999)}", n=rng.randint(1, 3),
                sz=rng.randint(100, 9999), span_id=f"{rng.randint(1000,9999):x}",
                max=50, id=rng.randint(1, 99), req=rng.randint(10, 500),
                handler=f"handle_{rng.choice(['get','post','delete'])}",
...
def gt_app_log(corpus_str: str, corpus_meta: dict) -> dict:
    ...
    anomaly_lines = [{"line_number": p, "content": lines[p]} for p in anomaly_positions if p < len(lines)]
...
def gt_source_file(corpus_str: str, _meta: dict) -> dict:
    ...
    defs = [(i, m.group(1), m.group(2)) for i, l in enumerate(lines) for m in [def_re.match(l)] if m]
...
def build_json_api(seed: int = SEED) -> tuple[str, dict, str]:
    ...
    for i in range(500):
        ...
        records.append({
            "id": i + 1, "org": f"org-{rng.randint(1000,9999)}",
            "plan": plan, "status": status, "region": rng.choice(regions),
            "seats": rng.randint(1, 500), "mrr_usd": round(rng.uniform(0, 50000), 2),
    ...
    meta = {
        "total_records": 500, "anomaly_ids": anomaly_ids,
        "plan_distribution": plan_dist, "status_distribution": status_dist,
    }
...
def gt_json_api(corpus_str: str, corpus_meta: dict) -> dict:
    ...
    anomaly_records = [r for r in records if r["status"] == "suspended" and r["plan"] == "enterprise"]
...
def build_stacktrace(seed: int = SEED) -> tuple[str, dict, str]:
    ...
    frames = [
        ...
        ("django/core/handlers/base.py", 197, "_get_response", "response = wrapped_callback(request, *callback_args, **callback_kwargs)"),
        ("myapp/views/api.py", 142, "dispatch", "return super().dispatch(request, *args, **kwargs)"),
        ("rest_framework/views.py", 509, "dispatch", "response = handler(request, *args, **kwargs)"),
        ...
        ("myapp/pipeline/steps/validate.py", 112, "_check_nested", "raise ValidationError(f'Invalid field at {path!r}: {value!r}')"),
    ]
    chained_frames = [
        ("django/core/handlers/exception.py", 55, "inner", "response = get_response(request)"),
        ("myapp/middleware/error_handler.py", 34, "process_exception", "return self.format_error(exc)"),
        ("myapp/middleware/error_handler.py", 67, "format_error", "body = json.dumps(self.serialize(exc))"),
        ("myapp/middleware/error_handler.py", 91, "serialize", "detail = exc.detail if hasattr(exc, 'detail') else str(exc)"),
        ("myapp/middleware/error_handler.py", 95, "serialize", "raise SerializationError('Cannot serialize exception detail') from exc"),
    ]
    ...
    lines.extend(["", "During handling of the above exception, another exception occurred:", "",
...
def gt_stacktrace(corpus_str: str, corpus_meta: dict) -> dict:
    ...
    frame_indices = [i for i, l in enumerate(lines) if frame_re.match(l)]
    exc_lines = [(i, l) for i, l in enumerate(lines) if l.startswith("myapp.exceptions.")]
    ...
    return {
        "anomaly": {
            ...
            "exceptions": [{"line_number": i, "content": l} for i, l in exc_lines],
