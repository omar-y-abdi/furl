...
### 🔮 FUTURE ROADMAP (user 2026-07-02 — logged, execute later; full detail in handoff §8)
1. **RENAME/REBRAND** — user dislikes "headroom-mcp"; wants to brainstorm a name first. Then EVERYTHING with "headroom" → new name: PyPI pkg, `headroom/` module, Rust crates, MCP server name, HEADROOM_* envs, README/docs/**badges+images**, GitHub repo name, pyproject. One comprehensive rename pass after the name is locked.
2. **CLAUDE PLUGIN (all-in-one, 2-command install)** — bundle MCP server + hooks (pre-tool-use compression) + skills (how-it-works) into a Claude Code plugin (`.claude-plugin/plugin.json` + hooks/ + skills/ + MCP registration). This IS the "MCP tool creation" north-star — after codebase is beyond-perfect-for-usage.
