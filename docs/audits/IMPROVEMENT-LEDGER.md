...
- 2026-07-21: SECURITY.md's two `furl-ctx[mcp]==X.Y.Z` engine pins are covered
  by neither tests/test_plugin_version_pins.py's file list nor the
  CONTRIBUTING.md "Releasing / version bumps" hand-sync list, which is how the
  1.3.2 release nearly shipped stale 1.3.0 pins in SECURITY.md until a grep
  caught them. Candidate: add SECURITY.md to both the pin-guard test's checked
  files and the CONTRIBUTING hand-sync list in a post-release PR, so the guard
  fails CI on a future drift instead of relying on a manual grep.
