...
- `CCRConfig.enabled=False` end-to-end — **closed; semantics corrected (Defect 1)**. Both `enabled=False` and `inject_retrieval_marker=False` collapse to the same Rust `advertise_retrieval_tool=False` field, which gates NEITHER the recovery pointer NOR the store write — persistence and pointer emission are unconditional on every drop. The flag pair is the retrieval-tool advertisement preference (plus the router's CCR-offload gate). See the SmartCrusher table above.
